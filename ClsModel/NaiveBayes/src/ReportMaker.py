...
def get_reports(true_answers,model_answers,file_extra_name,labels):
        report_file = open("./P2_Phase1/ClsModel/NaiveBayes/"+file_extra_name+".report.txt","w")

        labels_reports = []
        for label in labels:    
            report = {"tp":0,"tn":0,"fp":0,"fn":0,"recall":0,"precision":0,"F1":0}
            for index in range(len(true_answers)):
                if(true_answers[index] == label ):
...
dataSet_output_file = open("./P2_Phase1/ClsModel/NaiveBayes/Test.output.txt")
...
testCase_output_file = open("./P2_Phase1/ClsModel/NaiveBayes/TestCase.output.txt")
